...
/// <summary>
///     Interaction logic for BoolImageRect.xaml
/// </summary>
public partial class BoolImageRect : UserControl
{
    private static readonly ILog Logger = LogManager.GetLogger(nameof(BoolImageRect));
    ...
    public BoolImageRect(IActorRegistry actorRegistry, string actorName, string itemPath)
    ...
    public static readonly DependencyProperty RectWidthProperty =
        DependencyProperty.Register(nameof(RectWidth), typeof(double), typeof(BoolRectView),
    ...
    public static readonly DependencyProperty RectHeightProperty =
        DependencyProperty.Register(nameof(RectHeight), typeof(double), typeof(BoolRectView),
            new PropertyMetadata(18.0));

    public void SetImage(string relativePath)
    {
        var uriPath = $"pack://application:,,,/PnpExample;component/{relativePath}";

        if (!Uri.IsWellFormedUriString(uriPath, UriKind.Absolute))
        {
            Logger.Error($"Can not find uri path: {uriPath}");
            return;
        }

        try
        {
            var uri = new Uri($"pack://application:,,,/PnpExample;component/{relativePath}");
            ...
            BoolValueRect.Fill = brush;
        }
        catch (Exception ex)
        {
            Logger.Error($"Image resource not found. {relativePath}");
        }
    }
    ...
    private void Binder_DataChanged(object? sender, Dict e)
    {
        ...
        if (value is bool)
        {
            if (string.IsNullOrEmpty(_imagePath))
                ...
                BoolValueRect.Fill.Opacity = _value is true ? 0.3 : 1;
        }
    }
